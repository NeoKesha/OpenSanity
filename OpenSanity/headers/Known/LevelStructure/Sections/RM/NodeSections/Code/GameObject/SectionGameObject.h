...
class SectionGameObject : public SectionAbstract {
	public:
		SectionDataGameObjectBase * collection;
		IndexToIdList * indexToIdList;

		virtual SectionReaderGameObject* GetReader(uint id, int param_2);
		...
		virtual bool Write(MemoryStream* stream, uint* indexOut1, int* indexOut2);
		void Construct(uint collection, uint indexToIdList);
